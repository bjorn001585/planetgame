...
bot.on("ready", async () => {
    ...
    bot.user.setActivity("text", {type: "WATCHING"});     //Смотрит   {},
    //bot.user.setGame("text");                           //Играет в  {}.
});
...
bot.on("message", async message => {
    ...
    if (cmd === `${prefix}choose`){
        var m1 = Math.floor(Math.random() * 99) + 1;
        var m2 = Math.floor(Math.random() * 99) + 1;
        var m3 = Math.floor(Math.random() * 99) + 1;
        if (!args[0])
            return message.channel.send("\`\`\`!выбор [text]\`\`\`")
        let выбор = args.join(" ");
        let testEmbed = new Discord.RichEmbed()
        .setTitle("Я помог сделать выбор за " + message.member.displayName + " с текстом: " + (выбор))
        .setColor("#ffa500")
        .setDescription("Запустить - " + m1 + "% необходимости!\nПотом - " + m2 + "% необходимости!\nНе надо - " + m3 + "% необходимости!")
        .setFooter("Ильич");
        message.delete().catch(O_o=>{});
    ...
    if (cmd === `${prefix}news`) {
        if (!message.member.hasPermission("MANAGE_MESSAGES"))
            return message.channel.send("Тебе не разрешено это делать.")
        message.delete().catch(O_o=>{});
        let news = args.join(" ");
        let newsEmbed = new Discord.RichEmbed()
        .setTitle("Сгодня в новостях")
        .setColor("#0F0000")
        .setDescription("О сервере:\n" + (news))
        .setFooter("PlanetGame");
        let newschannel = message.guild.channels.find(`name`, "467372437395210251");
        if(!newschannel) return message.channel.send("Новость с текстом " + (news) + " была отправленна");
